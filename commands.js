...
async function generateCommitMessageCommand() {
  ...
  while (action === "Regenerate") {
    ...
    const selection = await vscode.window.showQuickPick(
      [
        { label: "$(check) Accept", description: "Use this message", detail: finalMessage },
        { label: "$(sync) Regenerate", description: "Try again" },
        { label: "$(copy) Copy", description: "Copy to clipboard" }
      ],
      {
...
async function toggleConventionalCommitsCommand() {
  const config = vscode.workspace.getConfiguration("commitMessageGenerator");
...
async function toggleAutoFillCommand() {
  const config = vscode.workspace.getConfiguration("commitMessageGenerator");
...
async function setCommitToneCommand() {
...
async function setCustomPromptCommand() {
  const config = vscode.workspace.getConfiguration("commitMessageGenerator");
...
async function toggleCustomPromptCommand() {
  const config = vscode.workspace.getConfiguration("commitMessageGenerator");
...
async function toggleCrossCheckCommand() {
  const config = vscode.workspace.getConfiguration("commitMessageGenerator");
